<template>
  <v-card
    class="content-card mx-auto"
    tile
    outlined
    width="100%"
    color="primary"
  >
    <v-img
      height="45%"
      v-if="media || img"
      :src="media ? media[0] : img"
      gradient="to bottom, rgba(0,0,0,.2), rgba(0,0,0,1)"
      class="align-end"
    >
      <v-card-title class="text-h5 mb-1 text-uppercase accent--text">
        {{ title }}
      </v-card-title>
    </v-img>
    <v-card-title
      v-if="!media && !img"
      class="text-h5 mb-1 text-uppercase accent--text"
    >
      {{ title }}
    </v-card-title>
    <v-card-subtitle v-if="date" class="accent--text">{{
      date
    }}</v-card-subtitle>
    <v-card-subtitle v-if="tech">
      <v-btn
        color="accent"
        class="no-events"
        outlined
        tile
        x-small
        v-for="(item, index) in tech"
        :key="index"
        >{{ item }}</v-btn
      >
    </v-card-subtitle>
    <v-card-text class="accent--text">
      {{ content }}
    </v-card-text>
    <v-card-actions v-if="contentType != 'about'" class="pa-4">
      <v-btn color="cta" tile outlined small>More</v-btn>
      <v-spacer></v-spacer>
      <div v-if="links">
        <v-btn
          v-for="(link, index) in links"
          :key="index"
          :class="link.disabled ? 'no-events' : ''"
          :href="link.url"
          :color="link.disabled ? 'accent darken-2' : 'cta'"
          target="_blank"
          tile
          outlined
          icon
          small
        >
          <v-icon>{{ link.icon }}</v-icon>
        </v-btn>
      </div>
    </v-card-actions>
  </v-card>
</template>
<style lang="scss" scoped>
.no-events {
  pointer-events: none;
}

.content-text {
  max-width: 250px;
  overflow-wrap: break-word;
  word-break: break-word;
}

.v-card {
  &.content-card {
    border: #d57e8e 2px solid !important;
    position: relative;
  }
  &__actions {
    position: absolute;
    bottom: 0;
    left: 0;
    right: 0;
  }
}
</style>
<script>
export default {
  name: "ProjectCard",
  props: {
    contentType: String,
    title: String,
    tech: Array,
    media: Array,
    links: Object,
    content: String,
    date: String,
    img: String,
  },
};
</script>
